...
public class _1035RefreshingtheSprings extends QuestHandler
{
	...
	public void register()
	{
		...
		qe.setNpcQuestData(203992).addOnTalkEvent(questId); //Ophelos
		qe.setNpcQuestData(700158).addOnTalkEvent(questId); //Laquepin Life Stone
		qe.setNpcQuestData(203965).addOnTalkEvent(questId); //Castor
		qe.setNpcQuestData(203968).addOnTalkEvent(questId); //Corybantes
		qe.setNpcQuestData(203987).addOnTalkEvent(questId); //Heratos
		qe.setNpcQuestData(700160).addOnTalkEvent(questId); //Desert Life Stone
		qe.setNpcQuestData(203934).addOnTalkEvent(questId); //Sirink
		qe.setNpcQuestData(700159).addOnTalkEvent(questId); //Temple Life Stone
	...
	public boolean onDialogEvent(QuestEnv env)
	{
		...
		else if(targetId == 203992) //Ophelos
		{
		...
		else if(targetId == 203965) //Castor
		{
		...
		else if(targetId == 203968) //Corybantes
		{
					...
					if (qs.getQuestVarById(0) == 2 && env.getDialogId() == -1)
					{
						...
						ThreadPoolManager.getInstance().schedule(new Runnable(){
							...
							public void run()
							{
								...
								itemService.addItems(player, Collections.singletonList(new QuestItems(182201014, 1)));
								qs.setQuestVar(3);
								updateQuestStatus(player, qs);
		...
		else if (qs.getStatus() == QuestStatus.START && qs.getQuestVarById(0) == 7)
		{
			switch(targetId)
			{
				case 700160: //Laquepin Life Stone
				{
		...
		else if (qs.getStatus() == QuestStatus.START && qs.getQuestVarById(0) == 10)
		{
			switch(targetId)
			{
				case 700159: //Temple Life Stone
				{
					if (qs.getQuestVarById(0) == 10 && env.getDialogId() == -1)
					{
						final int targetObjectId = env.getVisibleObject().getObjectId();
						PacketSendUtility.sendPacket(player, new SM_USE_OBJECT(player.getObjectId(), targetObjectId, 3000,
							1));
