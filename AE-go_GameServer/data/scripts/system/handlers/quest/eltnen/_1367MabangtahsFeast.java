...
public class _1367MabangtahsFeast extends QuestHandler
{
	...
	{
		qe.setNpcQuestData(204023).addOnQuestStart(questId); //Kobold Cook
		...
		if(targetId == 204023)
		{
			...
			else if(qs != null && qs.getStatus() == QuestStatus.START)
			{
				...
				switch(env.getDialogId())
				{
					...
					case 10000:
						itemCount = player.getInventory().getItemCountByItemId(182201331);
						if (itemCount > 0)
						{
							qs.setQuestVar(1);
							qs.setStatus(QuestStatus.REWARD);
							updateQuestStatus(player, qs);
							player.getInventory().removeFromBagByItemId(182201331, 1);
							return sendQuestDialog(player, env.getVisibleObject().getObjectId(), 5);
						}
						else
							return sendQuestDialog(player, env.getVisibleObject().getObjectId(), 1693);
					case 10001:
						itemCount = player.getInventory().getItemCountByItemId(182201332);
						if (itemCount > 0)
						{
							qs.setQuestVar(2);
							qs.setStatus(QuestStatus.REWARD);
							updateQuestStatus(player, qs);
							player.getInventory().removeFromBagByItemId(182201332, 1);
							return sendQuestDialog(player, env.getVisibleObject().getObjectId(), 6);
						}
						else
							return sendQuestDialog(player, env.getVisibleObject().getObjectId(), 1693);
					case 10002:
						itemCount = player.getInventory().getItemCountByItemId(182201333);
						if (itemCount > 0)
						{
							qs.setQuestVar(3);
							qs.setStatus(QuestStatus.REWARD);
							updateQuestStatus(player, qs);
							player.getInventory().removeFromBagByItemId(182201333, 1);
							return sendQuestDialog(player, env.getVisibleObject().getObjectId(), 6);
						}
						else
							return sendQuestDialog(player, env.getVisibleObject().getObjectId(), 1693);
